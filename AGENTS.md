...
- In-process queue: `play` replaces, `queue` appends, `next`/`prev`/`jump`,
  auto-advance. Shuffle reorders the remaining tail and keeps the current
  track. New appends and radio fills mix into that tail while shuffle is on.
...
- Commands: `login`, `search`, `favs`, `play`, `queue`, `radio`, `next`,
  `prev`, `jump`, `toggle`, `stop`, `shuffle`, `favorite`, `status`, `quit`,
  `doctor`, `daemon`. Versioned JSON output for Shell integration.
- Omarchy plugin `community.otidal` enabled on the right of the bar as a
  symlink to `omarchy-plugin/`. Popup: search, favorites, radio, queue,
  shuffle, favorite heart, transport. Lists clip inside the popup with a
  scrollbar. Long titles marquee on hover. Shuffle and Close List sit under
  the search field when a list is open. Close List hides the list; it does
  not empty the play queue.
